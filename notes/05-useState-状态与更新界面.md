...
- `count` —— **当前值**(这次渲染里的快照,这里是个数字)
- `setCount` —— **更新函数**:注意它不是"直接给 count 赋值",调用它是**预约一次重渲染**,把新值交给 React,等组件函数重新执行时,`count` 才拿到新值(详见下文「快照」一节)

`useState(0)` 里的 `0` 是初始值,只在组件**第一次**渲染时被采用。

这两个名字是**自己起的**:`useState` 返回的只是一个数组 `[当前值, 设置函数]`,靠数组解构取出来,所以叫 `count`/`setCount` 还是 `n`/`setN` 都行,只是惯例用 `setXxx` 命名第二个。解构展开前其实是这个样子:

```jsx
const arr = useState(0);
const count = arr[0];    // 当前值
const setCount = arr[1]; // 更新函数
```
